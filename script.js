...
function openInboxModal() {
    ...
    c.innerHTML = inboxAdmin.length === 0
        ? '<p>Tidak ada laporan.</p>'
        : inboxAdmin.map(i => `<p><b>${i.ref}</b> - ${i.name} (${i.date}) : ${i.notes}</p>`).join('');
...
function toggleAccess() {
    ...
    document.getElementById('accessStatus').innerText =
        "Akses Sekarang: " + (accessOpen ? "Dibuka" : "Ditutup");
    saveDataToFirebase();
}

function showPage(id) {
    ['homePage', 'bookingPage', 'returnPage', 'adminPage'].forEach(p => {
    ...
    ['bookingTable1', 'bookingTable2'].forEach(tid => {
        ...
        bookings.forEach(b => {
            ...
            const timeDisplay = (b.startTime && b.endTime)
                ? `<br><small>(${b.startTime} - ${b.endTime})</small>`
                : '';

            tr.innerHTML = `
                <td>${b.ref}</td>
                <td>${b.name}</td>
                <td>${b.date}${timeDisplay}</td>
                <td>${b.purpose}</td>
                <td>${b.laptop}</td>
                <td>${b.lcd}</td>
                <td>${b.accessory.join(', ')}</td>
            `;
...
function updateDropdowns() {
    ...
    bookings.forEach(b => {
        b.accessory.forEach(a => {
            if(a !== 'NONE' && accessoryStock[a] !== undefined) {
                accessoryStock[a]--;
            }
        });
    });
    ...
    for(let i = 0; i < laptopOptions.length; i++) {
        laptopOptions[i].disabled = laptopsBooked.includes(laptopOptions[i].value);
    }

    const lcdOptions = document.getElementById('lcd').options;
    for(let i = 0; i < lcdOptions.length; i++) {
        lcdOptions[i].disabled = lcdBooked.includes(lcdOptions[i].value);
    }
...
accessoryCheckboxes.forEach(cb => cb.addEventListener('change', function() {
    ...
    if(noneCheckbox.checked) {
        accessoryCheckboxes.forEach(c => {
            if(c.value !== 'NONE') c.disabled = true;
        });
    } else {
        accessoryCheckboxes.forEach(c => {
            if(c.value !== 'NONE') c.disabled = false;
        });
    }
}));
...
document.getElementById('bookingForm').addEventListener('submit', function(e) {
    ...
    if(!accessOpen) {
        alert('Akses Peminjaman ditutup');
        return;
    }
    ...
    let accessory = Array.from(accessoryCheckboxes)
        .filter(c => c.checked)
        .map(c => c.value);

    if([laptop, lcd].every(a => 'NONE' === a) && accessory.length === 0) {
...
document.getElementById('returnForm').addEventListener('submit', function(e) {
    ...
    if(!accessOpen) {
        alert('Akses Pemulangan ditutup');
        return;
    }
    ...
    if(index === -1) {
        alert('Reference code tidak ditemui!');
        return;
    }

    if(status === 'incomplete' && notes === '') {
        alert('Sila isi keterangan jika barang TIDAK lengkap');
        return;
    }
    ...
    if(status === 'incomplete') {
        ...
        if(adminLoggedIn) alert('Barang dikembalikan');
    } else {
        if(adminLoggedIn) alert('Barang berjaya dikembalikan lengkap');
    }
...
function checkReference() {
    ...
    if(rec) {
        const timeStr = (rec.startTime && rec.endTime)
            ? `<br><b>Masa:</b> ${rec.startTime} - ${rec.endTime}`
            : '';

        document.getElementById('refDetails').innerHTML = `
            <b>Reference:</b> ${rec.ref}<br>
            <b>Nama:</b> ${rec.name}<br>
            <b>Tarikh:</b> ${rec.date}${timeStr}<br>
            <b>Tujuan:</b> ${rec.purpose}<br>
            <b>Laptop:</b> ${rec.laptop}<br>
            <b>LCD:</b> ${rec.lcd}<br>
            <b>Aksesori:</b> ${rec.accessory.join(', ')}<br>
            <b>Status:</b> Sedang Dipinjam
        `;
    } else if(hist) {
        const timeStr = (hist.startTime && hist.endTime)
            ? `<br><b>Masa:</b> ${hist.startTime} - ${hist.endTime}`
            : '';

        document.getElementById('refDetails').innerHTML = `
            <b>Reference:</b> ${hist.ref}<br>
            <b>Nama:</b> ${hist.name}<br>
            <b>Tarikh Peminjaman:</b> ${hist.date}${timeStr}<br>
            <b>Tujuan:</b> ${hist.purpose}<br>
            <b>Laptop:</b> ${hist.laptop}<br>
            <b>LCD:</b> ${hist.lcd}<br>
            <b>Aksesori:</b> ${hist.accessory.join(', ')}<br>
            <b>Status:</b> ${hist.status}<br>
            <b>Keterangan:</b> ${hist.notes}
        `;
    } else {
        document.getElementById('refDetails').innerText =
            "Reference tidak ditemui atau lebih 60 hari.";
    }
}
    ...
    document.getElementById('historyContent').innerHTML = recent.length === 0
        ...
        : recent.map(h => `
            <div class="report-item">
                <b>${h.name}</b> (${h.ref}) <br>
                <span style="color: ${h.status === 'complete' ? '#00ff00' : '#ff4500'}">
    ...
    let tableRows = dataList.map((h, index) => {
        return `
            <tr>
                ...
                <td class="${h.status}">
                    ${h.status === 'complete' ? 'LENGKAP' : 'TIDAK LENGKAP'}
                </td>
                <td>${h.notes || '-'}</td>
            </tr>
        `;
...
function printFullReport() {
    ...
    if (recent.length === 0) {
        alert("Tiada rekod data pemulangan untuk dicetak!");
        return;
    }
    ...
    if (!reportWindow) {
        alert("Popup disekat browser. Sila benarkan popup untuk website ini.");
        return;
    }
